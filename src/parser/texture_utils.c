...
/*   Updated: 2024/05/29 13:12:13 by matlopes         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	process_texture_line(char *line, t_data *data, t_error *error)
{
	...
	if (ft_strncmp(line, "NO ", 3) == 0)
		process_texture(path, &data->map.tex[0], data, error);
	else if (ft_strncmp(line, "SO ", 3) == 0)
		process_texture(path, &data->map.tex[1], data, error);
	else if (ft_strncmp(line, "WE ", 3) == 0)
		process_texture(path, &data->map.tex[2], data, error);
	else if (ft_strncmp(line, "EA ", 3) == 0)
		process_texture(path, &data->map.tex[3], data, error);
	else
	{
